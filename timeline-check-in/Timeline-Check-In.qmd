...
Together, these visualizations connect our figures to the main project questions. The species-rank plot identifies the plants that most strongly characterize the habitat, the dominant-species plot shows how the top five species change through time, the proportional native/non-native plot shows how plant origin contributes to habitat identity, and the dominant non-native species plot shows which non-native plants are driving non-native cover across survey years. The total-cover plot then provides broader context for overall vegetation development across the monitoring period.

## Data cleaning and wrangling

We filtered the vegetation dataset to focus on salt marsh and closely related brackish marsh vegetation, then limited the surveys to August and September because late summer and early fall are when vegetation cover is expected to be highest and most comparable across years. We used `complete()` to make implicit absences explicit as zero percent cover so that species not recorded in a given transect-year are treated as absent rather than missing (this matters for calculating accurate transect-level means). We then joined the vegetation data with the metadata file so we could account for the number of quadrats sampled in each transect-year when computing mean cover. Finally, we filtered to native and non-native plant cover categories so our analyses focused on vegetation rather than bare ground, thatch, or other non-plant cover types.
...
saltmarsh <- veg |> 
  # Filtering to salt marsh and brackish marsh because these habitat types share
  # similar plant communities and together represent the core restored wetland area at NCOS
  filter(str_detect(habitat, "Salt Marsh") | str_detect(habitat, "Brackish Marsh")) |>
  # Restricting to August and September; late summer is when cover peaks and is most stable across years, reducing seasonal noise   in comparisons
...
saltmarsh_complete <- saltmarsh |> 
  # Using complete() to make implicit absences explicit: if a species was not
  # recorded in a transect-year, it is assigned zero percent cover rather than
  # being dropped from the dataset, which would inflate mean cover estimates
  ...
  ungroup() |>
  # Creating a composite key to join vegetation data with quadrat metadata
  ...
  # Dividing summed cover by the number of quadrats to get mean percent cover
  # per quadrat within each transect-year, which accounts for variation in
  # sampling effort across transects and years
...
saltmarsh_plants <- saltmarsh_complete |> 
  # Keeping only native and non-native plant cover; removing bare ground, thatch,
  # and other non-plant categories so all analyses focus on vegetation
  filter(cover_category %in% c("NATIVE COVER", "NON-NATIVE COVER")) |>
  # Replacing underscores with spaces in species codes so names render cleanly
  # in figure axes and legends without extra formatting steps
...
# Calculating the 90th percentile of species mean percent cover as the dominance threshold.
# We chose the 90th percentile rather than a visual cutoff because it provides a
# reproducible, data-driven criterion: only species whose average cover exceeds
# 90% of all recorded species are classified as dominant. This threshold identifies
# the highest-contributing species without arbitrarily inflating or deflating the set.
...
# for use in ordering the stacked bar plot legend and stack order
dominant_species_order <- species_rank |>
  filter(psoc %in% dominant_species) |>
  arrange(desc(mean_pc)) |>
  select(psoc, species_label, overall_mean_pc = mean_pc)

# Summarizing dominant species cover by year for Figure 1
dominant_by_year <- saltmarsh_plants |>
  ...
  ungroup() |>
  # Averaging across transects within each year and species so that
  # bar height reflects mean cover rather than summed cover across transects
  ...
  # Ordering species so the highest-cover species appears at the bottom of the stacked bar (most visually stable position)
  mutate(species_label = factor(species_label,
                                levels = rev(dominant_species_order$species_label)))

# Keeping the top 20 species for the rank plot, excluding singletons (n > 1)
# to avoid species with only one observation inflating apparent cover
species_rank_top20 <- species_rank |>
  filter(n > 1) |>
  slice_max(mean_pc, n = 20)

# Creating a color palette for dominant species; green shades for natives,
# coral/red shades for non-natives, to make origin immediately readable
...
# Creating transect-level native and non-native cover totals for underlying points in Figure 2
# Each row represents the total native or non-native cover within one transect-year,
# which is what we plot as raw observations behind the yearly means
...
# Summarizing to yearly means and standard errors for the line and ribbon in Figure 2
native_nonnative_raw <- saltmarsh_plants |>
  # First collapsing to transect-level totals so each transect-year
  # contributes one observation per cover category to the yearly mean
  group_by(year, transect_name, cover_category) |>
  summarize(total_pc = sum(mean_pc, na.rm = TRUE)) |>
  ungroup() |>
  # Then averaging across transects within each year and cover category
...
# Identifying the five non-native species with the highest average cover across all years;
# focusing on the top five keeps the stacked bar readable while capturing the species
# most responsible for non-native cover patterns in the marsh
...
# Summarizing top non-native species cover by year for Figure 3
top_nonnative_by_year <- saltmarsh_plants |>
  filter(cover_category == "NON-NATIVE COVER",
         psoc %in% top_nonnative_species) |>
  mutate(species_label = str_replace_all(psoc, "_", " ")) |>
  # First summarizing within transect-years
  group_by(year, transect_name, psoc, species_label) |>
  summarize(sum_pc = sum(mean_pc, na.rm = TRUE)) |>
  ungroup() |>
  # Then averaging across transects within each year
...
# Creating transect-level total vegetation cover values for the underlying
# raw data points in Figure 5; each row is one transect-year observation
total_cover_points <- saltmarsh_plants |>
  group_by(year, transect_name) |>
  summarize(total_pc = sum(mean_pc, na.rm = TRUE)) |>
  ungroup()

# Summarizing total vegetation cover by year for the line and ribbon in Figure 5
...
The following four figures directly address our project questions by connecting plant identity, cover, and origin through time. Figure 1 focuses on the dominant plant species overall, Figure 2 compares the contribution of native and non-native cover across years, Figure 3 examines which non-native species contribute most to non-native cover, and Figure 4 ranks the top species by mean cover to justify our dominant-species cutoff.
...
# Creating Figure 1: dominant species cover across survey years as a stacked bar plot.
# Stacking allows the total dominant-species cover and individual species contributions
# to be read from the same figure.
fig1 <- ggplot(data = dominant_by_year,
               aes(x = year,
                   y = mean_pc,
                   fill = species_label,
                   group = species_label)) +
  # Stacking bars so each color segment shows one dominant species' contribution
  geom_col(position = "stack") +
  # Applying custom species colors; breaks argument controls legend order
  # so it matches the stacking order (highest cover at bottom)
  scale_fill_manual(values = species_rank_palette,
                    breaks = dominant_species_order$species_label) +
  # Showing each survey year as its own x-axis label
...
**Figure 2: Native and non-native cover across survey years**
...
#| fig-height: 6
#| fig-width: 8
#| fig-cap: "Mean percent cover (± 1 SE) of native and non-native vegetation in salt marsh habitat at NCOS across survey years. Faint points show transect-level observations (one point per transect-year per cover category); lines connect yearly means; shaded bands represent ± 1 standard error. Cover categories are labeled directly on the figure. Native cover is shown in green; non-native cover in coral."

# Creating Figure 2: native and non-native vegetation cover over time.
# Using a line plot rather than stacked bars to emphasize temporal trend and
# allow both categories to share the same axis for direct comparison.

# First, creating a label dataset with the last year's values for direct labeling
fig2_labels <- native_nonnative_raw |>
  group_by(cover_category) |>
  filter(year == max(year)) |>
  ungroup() |>
  # Cleaning up the cover category text for display
...
fig2 <- ggplot(data = native_nonnative_raw,
               aes(x = year,
                   ...
                   fill = cover_category)) +
  # Faint jittered points show the underlying transect-level observations;
  # each point is one transect-year total for that cover category
  ...
  # Shaded ribbons show ± 1 SE around each yearly mean
  geom_ribbon(aes(ymin = mean_cover - se_cover,
                  ymax = mean_cover + se_cover),
              alpha = 0.2,
              color = NA) +
  # Lines connect yearly means to show temporal trend
  geom_line(linewidth = 1) +
  # Larger points mark the yearly mean values
  geom_point(size = 3) +
  # Direct labels at the end of each line using ggrepel to avoid overlap
  ...
  scale_fill_manual(values  = c("NATIVE COVER"     = "#1D9E75",
                                "NON-NATIVE COVER" = "#D85A30")) +
  scale_x_continuous(breaks = seq(2018, 2025, 1),
                     # Expanding right side slightly to make room for direct labels
                     expand = expansion(mult = c(0.02, 0.15))) +
  labs(x = "Survey year",
       y = "Mean percent cover (± 1 SE)") +
  theme_minimal() +
  # Removing legend since cover categories are labeled directly on the figure
...
#| fig-height: 6
#| fig-width: 10
#| fig-cap: "Mean percent cover of the five most common non-native plant species in the filtered salt marsh/brackish marsh dataset across survey years, averaged across transects within each year. Bar height reflects the summed mean cover of these non-native species in each year. Species were selected as the five with the highest mean percent cover across all survey years."

# Creating a red/coral color palette for the top non-native species;
# using a single hue family (red to light coral) distinguishes these species from
# the green native palette while keeping all non-native species visually grouped
...
fig_3 <- ggplot(data = top_nonnative_by_year,
                aes(x = year,
                    y = mean_pc,
                    fill = species_label)) +
  # Stacked bars show combined non-native cover and each species' contribution
  geom_col(position = "stack") +
...
#| fig-height: 6
#| fig-width: 8
#| fig-cap: "Mean percent cover (± 1 SE) of the top 20 plant species recorded in the filtered salt marsh/brackish marsh dataset, averaged across transects and survey years and ranked from highest to lowest mean cover. Faint points show individual transect-year observations for each species. Points on the pointrange show means; horizontal bars show ± 1 standard error. Green indicates native species; coral indicates non-native species. Dashed line marks the 90th percentile cutoff used to identify dominant species for Figure 1."

# Creating a transect-year level dataset for underlying raw points in Figure 4.
# Each point represents one transect-year mean cover value for that species,
# showing the distribution of observations behind the overall mean.
species_rank_raw <- saltmarsh_plants |>
  ...
  ungroup() |>
  # Keeping only species that appear in the top 20
  filter(psoc %in% species_rank_top20$psoc)

# Setting the x-axis location of the dashed cutoff line from the
# 90th percentile calculated in the dominant-species-cutoff chunk
...
fig4 <- ggplot(data = species_rank_top20,
               aes(x = mean_pc,
                   ...
                   color = cover_category)) +
  # Faint raw points show the spread of transect-year observations for each species,
  # so the reader can assess how variable cover is around the mean
  geom_point(data = species_rank_raw,
             ...
             size = 1.5,
             color = "gray50") +
  # Pointrange shows the overall mean and ± 1 SE for each species
  geom_pointrange(aes(xmin = mean_pc - se_pc,
                      xmax = mean_pc + se_pc)) +
  # Dashed vertical line marks the 90th percentile dominance cutoff;
  # species to the right of this line were selected as dominant for Figure 1
...
The dashed line represents the 90th percentile of mean percent cover across all species in the filtered dataset. Species to the right of this line were classified as dominant because their average cover exceeds that of at least 90% of all recorded plant species. This gives the dominant-species selection a clear mathematical basis rather than relying on a visual cutoff chosen by eye. Faint gray points behind each pointrange show the individual transect-year observations, illustrating how variable cover is for each species across sampling events.
...
#| fig-height: 6
#| fig-width: 8
#| fig-cap: "Total mean percent vegetation cover (native + non-native combined) in salt marsh habitat at NCOS across survey years. Faint points show transect-level observations (one point per transect-year); the line connects yearly means; the shaded band represents ± 1 standard error around the mean. A warm amber color is used because this figure combines native and non-native cover rather than representing either group alone."

# Creating Figure 5: total vegetation cover (native + non-native) across survey years.
# Using a warm amber (#C4820A) rather than the green or coral used for individual
# cover categories, because this figure combines both groups and should not visually
# suggest association with either native or non-native cover alone.

fig5 <- ggplot(data = total_cover,
               aes(x = year,
                   y = mean_total)) +
  # Faint jittered points show the transect-level total cover observations;
  # each point is one transect-year's combined native + non-native cover
  geom_point(data = total_cover_points,
             ...
             size = 1.8,
             position = position_jitter(width = 0.08, height = 0),
            # Warm amber distinguishes total cover from native (green) and non-native (coral) used in other figures
             color = "#C4820A") +
  # Ribbon shows ± 1 SE around the yearly mean
  geom_ribbon(aes(ymin = mean_total - se_total,
                  ymax = mean_total + se_total),
              fill = "#C4820A",
              alpha = 0.2) +
  # Line connects yearly mean total cover values
  geom_line(color = "#C4820A",
            linewidth = 1) +
  # Larger points mark the yearly means
